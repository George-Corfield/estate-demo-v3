...
function PropItem({ label, value }) {
  return (
    <div style={{ padding: 12, background: 'rgba(255,255,255,0.07)', borderRadius: 8 }}>
      <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.4)', marginBottom: 3, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</p>
...
export default function MobileTaskOverlay({ taskId, onBack }) {
  ...
  const statusBtnStyle = (s) => {
    ...
    let activeBg = 'rgba(19,236,19,0.18)', activeColor = 'var(--color-primary)', activeBorder = 'rgba(19,236,19,0.35)'
    if (s.value === 'paused') { activeBg = 'rgba(245,158,11,0.18)'; activeColor = '#f59e0b'; activeBorder = 'rgba(245,158,11,0.35)' }
    if (s.value === 'cancelled') { activeBg = 'rgba(239,68,68,0.18)'; activeColor = '#f87171'; activeBorder = 'rgba(239,68,68,0.35)' }
    return {
      padding: '7px 13px', borderRadius: 8, fontSize: 12, fontFamily: 'var(--font-body)',
      fontWeight: 600, cursor: 'pointer', transition: 'all 120ms ease',
      background: (isActive || isExpanding) ? activeBg : 'rgba(255,255,255,0.08)',
      color: (isActive || isExpanding) ? activeColor : 'rgba(255,255,255,0.55)',
      border: `1px solid ${(isActive || isExpanding) ? activeBorder : 'rgba(255,255,255,0.12)'}`,
    }
  }

  // ── dark-themed linked-item rows ──────────────────────────────────────────

  const fieldsList = (
    <div>
      <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>
        Fields ({taskFields.length})
      </p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {taskFields.map(f => (
          <button key={f.id} onClick={() => navigate('/fields', { state: { openFieldId: f.id } })}
            style={{ width: '100%', textAlign: 'left', padding: '8px 12px', background: 'rgba(255,255,255,0.07)', borderRadius: 8, color: 'rgba(255,255,255,0.8)', border: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            {f.name}
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>{f.sizeHectares} ha</span>
          </button>
        ))}
      </div>
    </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {task.assignedTo.map(name => {
          ...
          const avatar = (
            <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(19,236,19,0.15)', color: 'var(--color-primary)', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {name.split(' ').map(n => n[0]).join('')}
            </div>
          )
          return (isManager && member) ? (
            <button key={name} onClick={() => navigate('/staff', { state: { openStaffId: member.id } })}
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: 'rgba(255,255,255,0.07)', borderRadius: 8, color: 'rgba(255,255,255,0.8)', border: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)' }}>
              {avatar}{name}
            </button>
          ) : (
            <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>
              {avatar}{name}
            </div>
          )
        })}
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {task.assignedTeams.map(team => (
          <span key={team} style={{ fontSize: 12, padding: '5px 10px', borderRadius: 99, background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.12)', display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        {task.assignedMachinery.map(m => {
          ...
          return (match && isManager) ? (
            <button key={m} onClick={() => navigate('/machinery', { state: { openEquipmentId: match.id } })}
              style={{ width: '100%', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: 'rgba(255,255,255,0.07)', borderRadius: 8, color: 'rgba(255,255,255,0.8)', border: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)' }}>
              <span className="material-symbols-outlined" style={{ fontSize: 15, color: 'rgba(255,255,255,0.4)' }}>agriculture</span>{m}
            </button>
          ) : (
            <div key={m} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', background: 'rgba(255,255,255,0.07)', borderRadius: 8, color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>
              <span className="material-symbols-outlined" style={{ fontSize: 15, color: 'rgba(255,255,255,0.4)' }}>agriculture</span>{m}
            </div>
          )
        })}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 60 }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '6px 20px 8px', flexShrink: 0 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', marginBottom: 2 }}>
              {fieldLabel}
            </div>
            <h2 style={{
        ...
        {/* ── MINIMAL: due date + type + swipe hint ── */}
        {snap === 'minimal' && (
          <div style={{ padding: '0 20px 14px', flexShrink: 0, display: 'flex', gap: 12, alignItems: 'center' }}>
            {task.dueDate && (
            ...
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)' }}>{task.type}</span>
            <span style={{ marginLeft: 'auto', fontSize: 11, color: 'rgba(255,255,255,0.25)', display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        {/* ── MID: compact summary ── */}
        {snap === 'mid' && (
          <div style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 24px' }} className="custom-scrollbar">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>

              {/* Compact info row */}
              <div style={{ display: 'flex', gap: 8 }}>
                <PropItem label="Type" value={task.type} />
                <PropItem label="Due" value={task.dueDate ? `${formatShortDate(task.dueDate)} · ${formatRelativeDate(task.dueDate)}` : '—'} />
              </div>

              {task.description && (
                <div>
                  <p style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>Description</p>
                  <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', lineHeight: 1.5 }}>{task.description}</p>
                </div>
              )}

              <div style={{ paddingTop: 4, display: 'flex', justifyContent: 'center' }}>
                <button onClick={() => setSnap('full')} style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
        ...
        {/* ── FULL: dark tab bar + light content area ── */}
        {snap === 'full' && (
          <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', background: 'var(--color-surface-50)' }} className="custom-scrollbar">
              <div style={{ padding: '16px 16px 32px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                            ...
                            <div style={{ padding: 16, borderRadius: 'var(--radius-md)', border: '1px solid rgba(245,158,11,0.3)', background: 'rgba(245,158,11,0.04)' }}>
                              ...
                              <div style={{ marginBottom: 12 }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                  {PAUSE_REASONS.map(r => (
                                    <label key={r} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 'var(--radius-sm)', border: `1px solid ${pauseReason === r ? 'var(--color-amber-400, #f59e0b)' : 'var(--color-surface-300)'}`, background: pauseReason === r ? 'var(--color-amber-100, #fef3c7)' : 'white', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)', color: 'var(--color-slate-700)', transition: 'all 120ms ease' }}>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div style={{ padding: 12, background: 'var(--color-surface-100)', borderRadius: 'var(--radius-md)' }}>
                        <p className="text-label" style={{ color: 'var(--color-slate-400)', marginBottom: 4 }}>Due Date</p>
                        <p className="text-body" style={{ color: 'var(--color-slate-900)', fontWeight: 500, margin: 0 }}>{formatShortDate(task.dueDate)}</p>
                      </div>
                      <div style={{ padding: 12, background: 'var(--color-surface-100)', borderRadius: 'var(--radius-md)' }}>
                        <p className="text-label" style={{ color: 'var(--color-slate-400)', marginBottom: 4 }}>Priority</p>
                        <p className="text-body" style={{ color: 'var(--color-slate-900)', fontWeight: 500, margin: 0 }}>{task.priority.charAt(0).toUpperCase() + task.priority.slice(1)}</p>
                      </div>
                      <div style={{ padding: 12, background: 'var(--color-surface-100)', borderRadius: 'var(--radius-md)' }}>
                        <p className="text-label" style={{ color: 'var(--color-slate-400)', marginBottom: 4 }}>Type</p>
                        <p className="text-body" style={{ color: 'var(--color-slate-900)', fontWeight: 500, margin: 0 }}>{task.type}</p>
                      </div>
                      <div style={{ padding: 12, background: 'var(--color-surface-100)', borderRadius: 'var(--radius-md)' }}>
                        <p className="text-label" style={{ color: 'var(--color-slate-400)', marginBottom: 4 }}>Due</p>
                        <p className="text-body" style={{ color: 'var(--color-slate-900)', fontWeight: 500, margin: 0 }}>{formatRelativeDate(task.dueDate)}</p>
                      </div>
                    </div>
                    ...
                    {/* Light-bg linked items for full tab */}
                    {taskFields.length > 0 && (
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {taskFields.map(f => (
                            <button key={f.id} onClick={() => navigate('/fields', { state: { openFieldId: f.id } })}
                              style={{ width: '100%', textAlign: 'left', padding: '6px 12px', background: 'var(--color-surface-100)', borderRadius: 'var(--radius-sm)', color: 'var(--color-slate-600)', border: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)', display: 'flex', justifyContent: 'space-between' }}>
                              {f.name} <span style={{ fontSize: 12, color: 'var(--color-slate-400)' }}>{f.sizeHectares} ha</span>
                            </button>
                          ))}
                        </div>
                      </div>
                    ...
                    {task.assignedTo?.length > 0 && (
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {task.assignedTo.map(name => {
                            const member = staff.find(s => s.name === name)
                            const avatar = <div style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(78,140,53,0.15)', color: 'var(--color-green-600)', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{name.split(' ').map(n => n[0]).join('')}</div>
                            return (isManager && member) ? (
                              <button key={name} onClick={() => navigate('/staff', { state: { openStaffId: member.id } })} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 12px', background: 'var(--color-surface-100)', borderRadius: 'var(--radius-sm)', color: 'var(--color-slate-600)', border: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)' }}>{avatar}{name}</button>
                            ) : (
                              <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 12px', color: 'var(--color-slate-600)', fontSize: 13 }}>{avatar}{name}</div>
                            )
                          })}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {task.assignedMachinery.map(m => {
                            const match = machinery.find(eq => eq.name === m)
                            return (match && isManager) ? (
                              <button key={m} onClick={() => navigate('/machinery', { state: { openEquipmentId: match.id } })} style={{ width: '100%', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, padding: '4px 12px', background: 'var(--color-surface-100)', borderRadius: 'var(--radius-sm)', color: 'var(--color-slate-600)', border: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-body)' }}>
                                <span className="material-symbols-outlined" style={{ fontSize: 16, color: 'var(--color-slate-400)' }}>agriculture</span>{m}
                              </button>
                            ) : (
                              <div key={m} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 12px', background: 'var(--color-surface-100)', borderRadius: 'var(--radius-sm)', color: 'var(--color-slate-600)', fontSize: 13 }}>
                                <span className="material-symbols-outlined" style={{ fontSize: 16, color: 'var(--color-slate-400)' }}>agriculture</span>{m}
                              </div>
                            )
                          })}
                    ...
                    {/* Service Details (full mode only) */}
                    {task.type === 'Service' && (
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                          {task.status === 'done' && task.typeFields?.completionData && (() => {
                            ...
                            return (
                              <div style={{ marginBottom: 8, padding: 12, background: 'rgba(78,140,53,0.06)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(78,140,53,0.2)' }}>
                                ...
                                <div className="grid grid-cols-2 gap-2 mb-2">
                                  {[['Service Type', cd.serviceType], ['Hours', `${cd.hoursAtService}h`], ['Cost', `£${cd.cost.toFixed(2)}`], ['Completed', formatShortDate(cd.completedAt)]].map(([l, v]) => (
                                    <div key={l} style={{ padding: 10, background: 'var(--color-surface-100)', borderRadius: 'var(--radius-md)' }}>
                                      <p className="text-label" style={{ color: 'var(--color-slate-400)', marginBottom: 2 }}>{l}</p>
                                      <p className="text-body" style={{ color: 'var(--color-slate-900)', fontWeight: 500, margin: 0 }}>{v}</p>
                                    </div>
                                  ))}
                                </div>
                ...
                {/* Type Fields tab */}
                {activeTab === 'typeFields' && <TaskTypeFields task={task} />}
