...
function WidgetTray({ open, onClose, onToggleCalendar, expandedWidget, children }) {
  return (
    <div style={{
      position: 'absolute',
      top: 0,
      ...
      {expandedWidget ? (
        ...
        <>
          ...
          <div style={{
            ...
            <span style={{ fontWeight: 700, fontSize: 15, color: 'var(--color-slate-100)', textShadow: '0 1px 2px var(--color-slate-700)' }}>Overview</span>
            <div style={{ display: 'flex', gap: 4 }}>
              <button
                onClick={onToggleCalendar}
                style={{
                  background: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.5)',
                  cursor: 'pointer', display: 'flex', alignItems: 'center', padding: 5,
                  borderRadius: 'var(--radius-sm)', color: 'var(--color-slate-600)',
          ...
          {/* Widget cards */}
          <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 16px', display: 'flex', flexDirection: 'column', gap: 12, pointerEvents: 'auto' }}>
            {children}
          </div>
        </>
      )}
...
export default function OverviewPage() {
    ...
    fetch(url)
      ...
      .then(d => {
        const days = d.daily.time.map((date, i) => {
          // Hourly slice for this day (24 hours per day)
          const hStart = i * 24
          ...
          // Skin temp (soil_temperature_0cm) → min/max for the day
          ...
          // Soil moisture at noon — average of 0-1cm and 3-9cm for 0-10cm layer
          ...
          return {
            ...
            et0: d.daily.et0_fao_evapotranspiration[i] != null
              ? Math.round(d.daily.et0_fao_evapotranspiration[i] * 10) / 10
              : null,
            icon: info.icon,
            label: info.label,
  ...
  const renderExpandedContent = () => {
    switch (expandedWidget) {
      case 'weather': {
        const days = weather || []
        const tw = days[0]
        // Operational note derived from first 2 days
                      ...
                      <div key={day.date}>
                        <button
                          ...
                          style={{
                            ...
                            gap: 10,
                            padding: '10px 14px',
                            background: i === 0 ? 'rgba(19,236,19,0.04)' : 'none',
                            border: 'none',
                            borderBottom: isExpanded ? 'none' : '1px solid var(--color-surface-100)',
                            cursor: 'pointer',
                            textAlign: 'left',
                            transition: 'background 0.1s',
                          }}
                            ...
                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 3 }}>
                              ...
                              {day.skinMin != null && (
                                <span style={{ fontSize: 12, color: 'var(--color-slate-400)' }}>
                                  Skin {day.skinMin}–{day.skinMax}°C
                                </span>
                              )}
                            </div>
                            {/* Row 2: labelled stats */}
                        ...
                        {isExpanded && (
                          <div style={{
                            ...
                            {day.skinMin != null && (
                              <span><strong style={{ color: 'var(--color-slate-700)' }}>Skin temp:</strong> {day.skinMin}–{day.skinMax}°C</span>
                            )}
                            {day.soilM010 != null && (
                              <span><strong style={{ color: 'var(--color-slate-700)' }}>Soil moisture:</strong> {day.soilM010} m³/m³ (0–10cm) / {day.soilM1040} m³/m³ (10–40cm)</span>
                            )}
                            {day.soilT010 != null && (
                              <span><strong style={{ color: 'var(--color-slate-700)' }}>Soil temp (0–10cm):</strong> {day.soilT010}°C</span>
                            )}
                            {day.solarWm2h != null && (
                              <span><strong style={{ color: 'var(--color-slate-700)' }}>Solar radiation:</strong> {day.solarWm2h} W/m²·H</span>
                            )}
                          </div>
                        )}
                      </div>
  ...
  return (
    <div className="relative w-full h-full flex flex-col" style={{ overflow: 'hidden' }}>
