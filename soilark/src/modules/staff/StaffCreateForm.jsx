...
export default function StaffCreateForm({ onCancel, onSave, editData }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="card flex flex-col gap-4">
      <h3 className="text-heading-4" style={{ color: 'var(--color-slate-900)', margin: 0 }}>
        {editData ? 'Edit Staff Member' : 'Add Staff Member'}
      </h3>

      <div className="grid grid-cols-2 gap-3">
        <FormField label="Full Name *" error={errors.name}>
          <input type="text" value={form.name} onChange={e => update('name', e.target.value)} placeholder="e.g. John Smith" className="form-input" style={errors.name ? { borderColor: 'var(--color-amber-400)' } : undefined} />
        </FormField>
        <FormField label="Email *" error={errors.email}>
          <input type="email" value={form.email} onChange={e => update('email', e.target.value)} placeholder="e.g. john@example.com" className="form-input" style={errors.email ? { borderColor: 'var(--color-amber-400)' } : undefined} />
        </FormField>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <FormField label="Mobile *" error={errors.mobile}>
          <input type="tel" value={form.mobile} onChange={e => update('mobile', e.target.value)} placeholder="e.g. 07700 900123" className="form-input" style={errors.mobile ? { borderColor: 'var(--color-amber-400)' } : undefined} />
        </FormField>
        <FormField label="Address *" error={errors.address}>
          <input type="text" value={form.address} onChange={e => update('address', e.target.value)} placeholder="Full address" className="form-input" style={errors.address ? { borderColor: 'var(--color-amber-400)' } : undefined} />
        </FormField>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <FormField label="Emergency Contact Name *" error={errors.emergencyContactName}>
          <input type="text" value={form.emergencyContactName} onChange={e => update('emergencyContactName', e.target.value)} placeholder="Contact name" className="form-input" style={errors.emergencyContactName ? { borderColor: 'var(--color-amber-400)' } : undefined} />
        </FormField>
        <FormField label="Emergency Contact Number *" error={errors.emergencyContactNumber}>
          <input type="tel" value={form.emergencyContactNumber} onChange={e => update('emergencyContactNumber', e.target.value)} placeholder="Contact number" className="form-input" style={errors.emergencyContactNumber ? { borderColor: 'var(--color-amber-400)' } : undefined} />
        </FormField>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <FormField label="Start Date *" error={errors.startDate}>
          <input type="date" value={form.startDate} onChange={e => update('startDate', e.target.value)} className="form-input" style={errors.startDate ? { borderColor: 'var(--color-amber-400)' } : undefined} />
        </FormField>
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormField label="Hours/Week *" error={errors.hoursPerWeek}>
            <input type="number" step="0.5" value={form.hoursPerWeek} onChange={e => update('hoursPerWeek', e.target.value)} placeholder="40" className="form-input" style={{ ...(errors.hoursPerWeek ? { borderColor: 'var(--color-amber-400)' } : {}), fontFamily: 'var(--font-mono)' }} />
          </FormField>
...
function FormField({ label, error, children }) {
  return (
    <div>
      ...
      {children}
      {error && <p style={{ fontFamily: 'var(--font-body)', fontSize: 11, color: 'var(--color-amber-400)', margin: '4px 0 0' }}>{error}</p>}
    </div>
  )
}
